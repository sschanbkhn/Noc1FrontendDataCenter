// ArchiveModal.tsx - Improved with full features
...
const ArchiveModal: React.FC<ArchiveModalProps> = ({ show, onClose, onEdit, onAdd }) => {
  ...
  const loadArchiveData = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_CONFIG.BASE_URL}/configuration/archive-reports?page=1&pageSize=200&${params.toString()}`);
  ...
  const visibleColumns = archiveData.length > 0 ? Object.keys(archiveData[0]).filter((key) => key.toLowerCase() !== "id") : [];
