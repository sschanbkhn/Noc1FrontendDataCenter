  ...
  const MultiSelectDropdown: React.FC<MultiSelectDropdownProps> = ({ label, options, selectedValues, onChange, placeholder }) => {
    ...
    return (
      <div className="position-relative">
        <button type="button" className="form-select d-flex justify-content-between align-items-center" style={{ borderRadius: "8px" }} onClick={() => setIsOpen(!isOpen)}>
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="p-2 border-bottom">
              <input type="text" className="form-control form-control-sm" placeholder="🔍 Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onClick={(e) => e.stopPropagation()} />
            </div>

            {/* Options */}
            <div className="p-1">
              {filteredOptions.map((option) => (
                <div key={option} className="form-check p-2 hover-bg-light" style={{ cursor: "pointer" }} onClick={() => handleCheckboxChange(option)}>
                  <input className="form-check-input" type="checkbox" checked={selectedValues.includes(option)} onChange={() => handleCheckboxChange(option)} />
                  <label className="form-check-label ms-2" style={{ cursor: "pointer" }}>
                    {option}
                  </label>
                </div>
              ))}
              {filteredOptions.length === 0 && <div className="p-2 text-muted">No options found</div>}
            </div>
          </div>
        )}
      </div>
...
*/
  ...
  const generateMockData = () => {
    const provinces = ["Hà Nội", "TP.HCM", "Đà Nẵng", "Hải Phòng", "Cần Thơ"];
    const districts = ["Quận 1", "Quận 2", "Ba Đình", "Hoàn Kiếm", "Hải Châu"];
    ...
    return Array.from({ length: 100 }, (_, i) => ({
      id: i + 1,
      original_id: 1000 + i,
      period_start_time: `${String(Math.floor(Math.random() * 24)).padStart(2, "0")}:${String(Math.floor(Math.random() * 60)).padStart(2, "0")}:00`,
      mrbts_name: `MRBTS_${String(i + 1).padStart(3, "0")}`,
      lnbts_name: `LNBTS_${String(i + 1).padStart(3, "0")}`,
      lncel_name: `CELL_${String(i + 1).padStart(3, "0")}`,
      dn_mrbts_site: `Site_${String(i + 1).padStart(3, "0")}`,
      pdcp_volume_dl: Math.floor(Math.random() * 10000000),
      pdcp_volume_ul: Math.floor(Math.random() * 5000000),
      cell_avail: +(Math.random() * 100).toFixed(2),
      max_ues: Math.floor(Math.random() * 1000),
      max_pdcp_dl: Math.floor(Math.random() * 50000000),
      max_pdcp_ul: Math.floor(Math.random() * 25000000),
      original_created_at: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString(),
      province: provinces[Math.floor(Math.random() * provinces.length)],
      district: districts[Math.floor(Math.random() * districts.length)],
      region: regions[Math.floor(Math.random() * regions.length)],
      data_date: selectedDate,
      data_year: new Date(selectedDate).getFullYear(),
      data_month: new Date(selectedDate).getMonth() + 1,
      data_day: new Date(selectedDate).getDate(),
      data_quarter: Math.ceil((new Date(selectedDate).getMonth() + 1) / 3),
      data_week: Math.ceil(new Date(selectedDate).getDate() / 7),
      vendor: vendors[Math.floor(Math.random() * vendors.length)],
      archived_at: new Date().toISOString(),
      archived_by: "SYSTEM_AUTO",
    }));
  };


  */
  //localhost:7232/api/monitoring/kpi-monitor/
  // Thay mock data bằng API call
  /*
  https: const fetchData = async () => {
    const response = await fetch(`localhost:7232/api/monitoring/kpi-monitor/${selectedDate}?page=${page}&pageSize=${pageSize}`);
  ...
  */
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await fetch(`https://localhost:7232/api/dashboard/kpi-monitor/${selectedDate}`);
        const result = await response.json();
  ...
  */
  ...
  /*
  // Function để fetch data khi click Display
  const handleDisplayClick = async () => {
    setLoading(true);
    try {
      const response = await fetch(`https://localhost:7232/api/monitoring/kpi-monitor/${selectedDate}`);
      const result = await response.json();
